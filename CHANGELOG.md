...
### 變更 (Changed)
- **優化跳轉速度**: 為了提升使用者體驗，將上標籤成功後的跳轉延遲從 300ms 縮短至 100ms。
- **縮短超時保護**: 將連線超時等待時間從 5 秒縮短至 2 秒，確保在網路不穩或伺服器異常時能更快導向目標網頁。
- **優化錯誤處理**: 連線失敗時的自動跳轉延遲從 3 秒縮短至 500ms，大幅減少使用者的等待感。
- **UI 調整**: 修改標題與狀態文字，讓頁面在處理中顯得更簡潔，並預設隱藏細節資訊以聚焦於「處理中」的狀態。
