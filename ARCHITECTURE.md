...
5.  收到成功發送後，等待 100 毫秒。若有設定 `redirect` 參數，則跳轉至指定網頁；否則呼叫 `liff.closeWindow()` 關閉網頁。
6.  **例外處理與超時保護**：若發生 `connect_error` 或是連線卡住超過 2 秒，系統將視為超時並強制作為中繼站將使用者跳轉至 `redirect` 目標（如果有的話），以避免畫面永久卡死。在發生錯誤的情況下，跳轉延遲縮短至 500 毫秒。
